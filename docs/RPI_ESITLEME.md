...
| A12 | **docker log döndürme** | ✅ | ✅ *(24 Ağu, drone2 yeni oluşum — 10m×3 ölçüldü)* | ✅ *(30 Ağu ÖLÇÜLDÜ — belge ❌ diyordu, YANLIŞTI)* | `run_drone.sh` içinde — **yalnız konteyner YENİDEN OLUŞTURULUNCA** devreye girer |
...
| A19 | ✅ **`-e ROS_LOCALHOST_ONLY=1` konteyner ortamında** | ✅ *(30 Ağu recreate)* | ✅ *(30 Ağu)* | ✅ *(30 Ağu)* | `run_drone.sh`'te (26 Ağu `dd5a1e6`) — **yalnız konteyner YENİDEN OLUŞTURULUNCA** devreye girer, tıpkı A12 gibi. O ana kadar elde `docker exec -e ROS_LOCALHOST_ONLY=1 ...` verilmeli · `TUZAKLAR` §1.25. **A12 ylp02'de ❌ — tek recreate ikisini birden kapatır** |
...
| A23 | ✅ **İkinci RC alıcısı (Görev 2 sürü kumandası) — ÇALIŞIYOR** | ✅ **30 Ağu ölçüldü** | ❌ | ❌ | **30 Ağu, operatör ylp00'a fiziksel olarak taktı.** FS-iA6B **i-BUS Servo** çıkışı → jumper → **fiziksel pin 29 (GPIO5) = uart2 RX** + GND. ✅ **Sinyal ölçüldü: ~3 V** → 3,3 V mantık, **seviye çevirici GEREKMİYOR** (Pi 5 GPIO'su 5 V toleranslı değil, bu yüzden ölçüldü). Kumanda `IntV1 5,3 V` gösteriyor = alıcının BESLEMESİ, sinyal değil; telemetri geldiğine göre **bind tamam**. ✅ `dtoverlay=uart2-pi5` eklendi + reboot → `/dev/ttyAMA2` oluştu. **Ölçüldü: 130 Hz, 0 checksum hatası, 0 atılan bayt**; 10 kanal, SwC 3 konumlu (1000/1500/2000). İşaret yönleri de ölçüldü — pitch ve yaw TERSTİ, kodda düzeltildi (`mode_manager/rc_eksen.py`). ✅ **Konteyner 30 Ağu 13:30'da yeniden oluşturuldu** — `--device /dev/ttyAMA2` geçti, konteyner içinden `crw-rw---- root:dialout 204,66` olarak görünüyor. Ayrıntı: `gorev2.md` §2 saha ölçümü |
...
| B25 | 🔴 **`SURU_KALKIS_OLAYLA=true` — KALKIŞ OTORİTESİ DEVREDİLDİ** | ✅ *(2 Eyl 06:30)* | ❌ | ✅ *(2 Eyl 06:30)* | `/ws/ucus_ayarlari.env`'e satır + `docker restart`. **Uçağa özgü DEĞİL, ama env dosyası `dagit.sh` ile GİTMEZ — elle eklenir.** 19 Ağustos'tan beri `false`'tu: görev başlayınca `agent_fsm` ARM ediyor ama `takeoff` göndermiyordu, kalkış emrini YKİ'nin guided yolu verecekti. Dağıtık mimaride o yol kalkınca kalkış emrini veren kimse kalmadı — ölçüldü: ylp00 25 sn `ARMED bekliyor: mission_start=False` yazıp yerde bekledi. Aynı satırda `GOREV_KALKIS_IRTIFA=10.0` da eklendi (agent_fsm `target_altitude_m` + mission1 `kalkis_irtifa_m`, **ikisi aynı olmak zorunda**) |
| B26 | 🔴 **Kod — 2 Eylül 07:00: görev node'u kalkış kararını devraldı** | ✅ | ❌ | ✅ | `dagit.sh --paket swarm_missions,swarm_state_machine` + restart. Üç parça: ① `mission1_node._kalkis_denetle()` irtifayı **home'a göreli** ölçüp `/swarm/internal/mission/kalkis_tamam` yayınlıyor ② `agent_fsm_node._on_kalkis_tamam` dinliyor ③ `agent_transitions._from_takeoff` `pending_state`'i okuyor. Neden: `agent_health_monitor` irtifayı **NED origin'den** ölçüyordu, origin yerde değil (ölçüldü: ylp00 arm z=1.56 → 1,06 m açık; ylp02 0,80 m). Uçaklar 10,0 m'ye çıkıp stabil durdu ama "ulaştım" hiç diyemedi → 30 sn timeout → FAILSAFE. **Aynı hatanın 3. kopyası** (esp32_bridge POSE, collision_avoidance irtifa kapısı). Ayrıca FAILSAFE artık sebebini yazıyor (`healthy/offboard/pil/px4_link/xy/z/vxy`) |
| B27 | 🔴 **Kod — 2 Eylül 08:00: hedefsiz setpoint boşluğu kapatıldı** | ✅ | ❌ | ✅ | `dagit.sh --paket swarm_missions` + restart. `orchestrator._on_rotate` ve `_on_navigate` QR konumu çözülemeyince `return None` diyordu → komut üreten kimse yok → `passthrough=0` → PX4 OFFBOARD'ı bırakıyor → `FAILSAFE SEBEBI: ... offboard=False`. 2 Eylül'de dört uçuşun üçü böyle bitti; operatör "kalktılar, asılı kaldılar, bir şey yapmadılar" diye bildirdi. Yeni `_hedefsiz_tut()` mevcut merkez + mevcut irtifada formasyonu KORUYAN komut üretiyor: setpoint akışı kesilmiyor, OFFBOARD yaşıyor, NAVIGATE zaman aşımı RETURN_HOME'a götürüyor. **Hedefli yol değişmedi** (yerde doğrulandı) |
| B28 | 🔴 **Kod — 2 Eylül 08:00: QR konum tablosunun MESH yolu** | ✅ | ❌ | ✅ | `dagit.sh --paket swarm_control` + restart. ⚠️ **BAZ İSTASYONU DA GEREKİR** — gönderen taraf laptoptaki `esp32_base`; `./src/gcs/yki_durdur.sh && ./src/gcs/yki_baslat.sh` şart, yoksa değişiklik yüklenmez. Alıcı (`_isle_qr_coords`) 30 Temmuz'dan beri vardı, **paketleyici hiç yazılmamıştı**: YKİ tabloyu `/swarm/internal/mission/qr_coords`'a latched yayınlıyor ve o konuya ABONE KİMSE YOKTU — "Drone'lara Gönder" boşluğa basıyordu. Sonucu: `route_unknown=True`, sürü hedefsiz. Yeni `pp.qr_koord_paketle()` + baz aboneliği (`_ORIGIN_QOS`, QoS eşleşmesi ŞART). Her QR ayrı 16 baytlık çerçeve; tek paket kaybı tabloyu tamamlatmaz (4 kopyalı tekrar bu yüzden) |
| B29 | 🔴 **`BATARYA_KESME=false` — PİL FAILSAFE'İ KAPATILDI** | ✅ *(2 Eyl 08:30)* | ❌ | ✅ *(2 Eyl 08:30)* | Kod: `dagit.sh --paket swarm_state_machine` + **env satırı elle**: `BATARYA_KESME=false` → `/ws/ucus_ayarlari.env` → restart. **İZLEME KAPALI DEĞİL** — pil ölçülmeye, YKİ'de görünmeye ve `Kritik batarya: X.XV` uyarısı üretmeye devam eder; yalnız `agent_fsm`'i FAILSAFE'e düşüren dal kapanır (`AgentContext.pil_kesme_aktif`). Neden: 13.8 V eşiği SAĞLAM pilde ölçülen 0,57 V çöküşe göre ayarlanmıştı; **boşalmış pilde ölçülen çöküş 1,26-1,31 V** (ylp00 %33=15,06 V→13,80 V · ylp02 %35=15,11 V→13,80 V), yani yarım pille her geliştirme uçuşu henüz bitmemiş pille kendini kesiyordu. Kill switch, EKF, PX4 link ve **PX4'ün kendi düşük-pil failsafe'i** aynen duruyor. Yan kazanç: `check()` artık uyarıları düşürmüyor ("Batarya dusuk" öteden beri sessizce kayboluyormuş), log 5 sn kısıtlı. 🔴 **YARIŞMA GÜNÜ `true` YAPILACAK** |
| B30 | **`GOREV_ROTA_BILINMEYEN_S=10.0` — hedefsiz bekleme kısaltıldı** | ✅ *(2 Eyl 08:50)* | ❌ | ✅ *(2 Eyl 08:50)* | Kod: `dagit.sh --paket swarm_state_machine` + **env satırı elle**: `GOREV_ROTA_BILINMEYEN_S=10.0` → restart. `_ROUTE_UNKNOWN_GRACE_S` kod içinde 30 sn sabitti; artık `ctx.rota_bilinmeyen_s` ile eziliyor (`navigate_timeout_s` ile aynı desen). Neden: QR tablosu yokken sürü B27 sayesinde formasyonu koruyor ama **dışarıdan "takıldı" ile ayırt edilemiyor**; 30 sn hareketsizlik operatörün sabrını aşıyor ve iki uçuş elle kesildi (2 Eyl). 10 sn tutmanın çalıştığını göstermeye yetiyor. 🔴 **YARIŞMA GÜNÜ 30.0'a alınacak** — QR tablosu varsa bu yol zaten işletilmez (`route_unknown=false`), ama geçici tablo kaybında sürü hemen eve dönmemeli |
| B31 | 🔴 **Kod — 2 Eylül 21:30: TEK-YAYINCI + RETURN_HOME başlığı** | ✅ | ✅ | ✅ | `dagit.sh --paket swarm_missions` + `docker restart`. **`.surum` üçünde de `commit=87812f7`** (temiz, +KIRLI yok). `tek_yayinci.py` zaten uçaktaydı, bu dağıtımla depo sürümü oldu |
| B32 | 🔴 **`ucus_ayarlari.env` — `SURU_KADRO="1 2 3"` · `SURU_BEKLENEN_UCAK=3`** | ✅ | ✅ | ✅ | 2 Eyl 21:00. **env md5 üçünde de `38ac72c1`** (birebir aynı). Yedek: `ucus_ayarlari.env.2eylul_oncesi`. Doğrulama: açılış logunda `beklenen ucak=3` + `kadro=1 2 3`. ⚠️ Önce ylp00/ylp02'de `2` idi — üçüncü uçak `formation_reached`'te sessizce yok sayılıyordu |
