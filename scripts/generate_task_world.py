...
def generate_random_qr_data():
    """Rastgele QR içeriklerini ve rotalarını üretir."""
    ...
    for qr_id in range(1, 7):
        ...
        irtifa_deger = (
            random.choice(list(range(15, 31))) if irtifa_aktif else 0
        )

        ayrilma_aktif = random.choice([True, False])
        ayrilacak_drone_id = random.randint(1, 3) if ayrilma_aktif else None
        hedef_renk = (
            random.choice(["KIRMIZI", "MAVI"]) if ayrilma_aktif else None
        )
...
def generate_qr_images(output_dir):
    """Rastgele QR PNG ve JSON dosyalarını üretir."""
...
def generate_task1_sdf():
    ...
    print("[1/2] QR kodları ve JSON dosyaları oluşturuluyor...")
    ...
    try:
        ...
        print(f"Hata: base_world.sdf bulunamadı. Yol: {base_world_path}")
        return
...
"""
    ...
    <model name="landing_pad_red">
      ...
      <link name="link">
        <visual name="visual">
          <geometry>
            <cylinder><radius>0.5</radius><length>0.01</length></cylinder>
          </geometry>
          <material>
...
"""
    ...
    if insertion_point != -1:
        ...
        print("  - JSON ve PNG dosyaları 'sim/qr/' klasörüne kaydedildi.")
