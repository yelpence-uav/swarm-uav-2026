"""test_rf_model.py - ESP-NOW RF modeli birim testleri."""
...
def test_kayip_yakin_sifir():
    """Yakin mesafede (<=50 m) kayip olasiligi 0."""
...
def test_cutoff_tam_kopma():
    """cutoff_m (450 m) otesi tam kopma."""
    rf = ESPNowRFModel(seed=1)
...
def test_yakinda_asla_dusmez():
    """0 m'de should_drop hep False."""
    rf = ESPNowRFModel(seed=1)
    assert all(
        not rf.should_drop_packet(0.0) for _ in range(200)
    )


def test_uzakta_hep_duser():
    """cutoff otesinde should_drop hep True."""
    rf = ESPNowRFModel(seed=1)
    assert all(
        rf.should_drop_packet(1000.0) for _ in range(200)
    )


def test_jitter_araligi():
    """Jitter 5-50 ms arasinda kalir."""
    rf = ESPNowRFModel(seed=1)
    for _ in range(200):
        j = rf.get_jitter()
...
def test_seed_tekrarlanabilir():
    """Ayni seed -> ayni jitter dizisi."""
    a = ESPNowRFModel(seed=42)
    b = ESPNowRFModel(seed=42)
    assert [
        a.get_jitter() for _ in range(10)
    ] == [
        b.get_jitter() for _ in range(10)
    ]
...
def test_distance_3d_irtifa_farki():
    """Ayni yatay konum, 10 m irtifa farki -> 3D mesafe ~10 m."""
    rf = ESPNowRFModel(seed=1)
    d = rf.distance_m(
        (41.0, 29.0, 100.0), (41.0, 29.0, 110.0)
    )
