...
class SynchronizedSwarmCommander(Node):
    def __init__(self):
        ...
        self.get_logger().info(
            f"🚀 SENKRONİZE KOMUTAN BAŞLADI: {len(self.drones)} İHA hatta bağlanıyor..."
        )
    ...
    def timer_callback(self):
        ...
        if self.counter < 100:
            if self.counter % 20 == 0:
                ...
                self.get_logger().info(
                    f"⏳ Sürü senkronize ediliyor... {kalan} saniye kaldı."
                )

        # 3. KISIM: HAKEMİN DÜDÜĞÜ! (Tam 10. saniyede herkese aynı anda ateşle komutu)
        elif self.counter == 100:
            self.get_logger().info(
                "🔥 TÜM SENSÖRLER HAZIR! SÜRÜ AYNI ANDA KALKIŞ YAPIYOR! 🔥"
            )
            for i in self.drones:
